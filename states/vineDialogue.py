...
class VineDialogue(State):
    def __init__(self, game):
        State.__init__(self, game)
        self.initialize_state()

    def initialize_state(self):
        self.game.vine_dialogue_bgm.play(-1)
        self.load_assets()
        # Other parts sa code
        ...
        file = open('assets/dialogues/afterMaze.txt', 'r')
        ...
        if keys[pygame.K_SPACE]:
            if not self.space_pressed:  # Only trigger when space was not pressed before
                ...
                if self.current_dialogue_number == len(self.dialogue):
                    newState = VineState(self.game)
                    self.exit_state()
                    self.game.vine_dialogue_bgm.stop()
    ...
    def load_assets(self):
        ...
        self.bg = self.game.load_background_asset("pngs/vinebg.jpg")
        # Example
